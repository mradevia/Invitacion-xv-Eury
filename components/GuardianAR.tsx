...
export const GuardianAR = () => {
  ...
  return (
    // ...
    onError = {(e: any) => {
  console.error("Error cargando modelo AR (inline):", e);
  setHasError(true);
}}
// ...
{/* Error Message */ }
  ...
  hasError && (
    <div className="absolute inset-0 flex items-center justify-center bg-royal-900/80 backdrop-blur-sm z-20">
      <div className="text-center p-6 border border-red-500/30 rounded-xl bg-red-500/10">
        <p className="text-red-400 font-bold mb-2">⚠️ No se pudo cargar el modelo</p>
        <p className="text-red-300/70 text-sm">Verifica la conexión o intenta recargar</p>
      </div>
    </div>
  )
}
        ...
        <div className="glass-royal rounded-2xl overflow-hidden border border-gold-500/20 shadow-[0_0_50px_rgba(212,175,55,0.15)]">
          {shouldLoadModel ? (
            <model-viewer
              ...
              reveal="auto"
              onError={(e: any) => {
                console.error("Error cargando modelo AR:", e);
                setLoadingProgress(0);
                // Podríamos mostrar un estado de error aquí
              }}
              style={{
                width: '100%',
                height: '400px',
